...
def get_items(id=None):
    ...
    if id == None:
        ...
        rows = cursor.execute(f"select * from item, details where id={id}")
...
def get_details():
    cursor = connection.cursor()
    rows = cursor.execute("select * from details")
    rows = list(rows)
    rows = [ {'orderNumber':row[0], 'quantity':row[1], 'cakeType':row[2]} for row in rows ]
    return rows
...
def setup_database():
    ...
    # cursor.execute("create table details(orderNumber integer, quantity num, cakeType text, foreign key (cakeType) references item(description))")
...
#     for description in ['Dutch Pastry', 'StrawBerry Shortcake', 'Blueberry Cheesecake', 'Pound Cake', 'Banana Bread']:
